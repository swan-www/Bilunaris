...
#include "../../../Xbox/Common_3/Graphics/Direct3D12/Direct3D12X.h"
    ...
    typedef void (*ztf_LogFn)(LogLevel, const char*, const char*);
...
#define GPU_MARKER_VALUE(markerBuffer, offset) (*((uint32_t*)markerBuffer->pCpuMappedAddress) + ((offset) / GPU_MARKER_SIZE))
    ...
    typedef struct DEFINE_ALIGNED(ztf_Cmd, 64)
    {
        ...
        union
        {
            ...
            struct
            {
                ...
                ztf_DescriptorSet* pBoundDescriptorSets[DESCRIPTOR_UPDATE_FREQ_COUNT];
            ...
            struct
            {
                ...
                ztf_DescriptorSet* mBoundDescriptorSets[DESCRIPTOR_UPDATE_FREQ_COUNT];
...
#endif
        ztf_PipelineReflection* pReflection;
    ...
    {
        ztf_VertexBinding mBindings[MAX_VERTEX_BINDINGS];
        ztf_VertexAttrib  mAttribs[MAX_VERTEX_ATTRIBS];
    ...
    MAKE_ENUM_FLAG(uint32_t, ztf_PipelineCacheFlags);
    ...
    {
        ztf_ShaderStats mStats[SHADER_STAGE_COUNT];
    } ztf_PipelineStats;
#endif
...
#endif
        ztf_GpuInfo  mGpus[MAX_MULTIPLE_GPUS];
        uint32_t mGpuCount;
    } ztf_RendererContext;
    ...
    {
        uint32_t             mRenderTargetCount;
        ztf_BindRenderTargetDesc mRenderTargets[MAX_RENDER_TARGET_ATTACHMENTS];
...
#define ZTF_DECLARE_RENDERER_FUNCTION(ret, name, ...)       \
    typedef ret(ZTF_FORGE_CALLCONV* ztf_name##Fn)(__VA_ARGS__); \
    ZTF_C_RENDERER_API extern ztf_name##Fn name;
    ...
    ZTF_DECLARE_RENDERER_FUNCTION(void, updateDescriptorSet, ztf_Renderer* pRenderer, uint32_t index, ztf_DescriptorSet* pDescriptorSet, uint32_t count, const DescriptorData* pParams)
    ...
    ZTF_DECLARE_RENDERER_FUNCTION(void, cmdBindPipeline, ztf_Cmd* pCmd, Pipeline* pPipeline)
    ...
    ZTF_DECLARE_RENDERER_FUNCTION(void, cmdWriteMarker, ztf_Cmd* pCmd, const MarkerDesc* pDesc);
    /************************************************************************/
